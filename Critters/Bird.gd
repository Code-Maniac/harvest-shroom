...
func _handle_fly(delta):
	...
	var fly_position = flight_path.points[fly_index]
	if global_position.distance_to(fly_position) < 3:
		...
		global_position = move_and_slide(velocity)
...
func _on_PlayerDetector_body_entered(_body:Node):
	...
	if new_fly_index > fly_index:
		...
		var fly_position =  flight_path.global_position + flight_path.points[fly_index]
		print(fly_position)
